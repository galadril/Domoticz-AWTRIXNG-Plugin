...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if "AWTRIXNG-Icons" not in Images:
            Domoticz.Image("AWTRIXNG-Icons.zip").Create()

        if UNIT_POWER not in Devices:
            Domoticz.Device(Name="Power", Unit=UNIT_POWER, TypeName="Switch", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_LUX not in Devices:
            Domoticz.Device(Name="Lux", Unit=UNIT_LUX, TypeName="Lux", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_TEMPHUM not in Devices:
            Domoticz.Device(Name="Temp+Hum", Unit=UNIT_TEMPHUM, TypeName="Temp+Hum", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_NOTIFICATION not in Devices:
            Domoticz.Device(Name="Send Notification", Unit=UNIT_NOTIFICATION, TypeName="Text", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_CUSTOMAPP not in Devices:
            Domoticz.Device(Name="Send Custom App", Unit=UNIT_CUSTOMAPP, TypeName="Text", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_SETTINGS not in Devices:
            Domoticz.Device(Name="Send Settings", Unit=UNIT_SETTINGS, TypeName="Text", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_NEXTAPP not in Devices:
            Domoticz.Device(Name="Next App", Unit=UNIT_NEXTAPP, TypeName="Switch", Switchtype=9, Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_PREVAPP not in Devices:
            Domoticz.Device(Name="Previous App", Unit=UNIT_PREVAPP, TypeName="Switch", Switchtype=9, Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_DISMISS not in Devices:
            Domoticz.Device(Name="Dismiss Notification", Unit=UNIT_DISMISS, TypeName="Switch", Switchtype=9, Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_RTTTL not in Devices:
            Domoticz.Device(Name="RTTTL", Unit=UNIT_RTTTL, TypeName="Text", Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_TRANSITION not in Devices:
            ...
            Domoticz.Device(Name="Transition effect", Unit=UNIT_TRANSITION, TypeName="Selector Switch", Options=options, Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_OVERLAY not in Devices:
            ...
            Domoticz.Device(Name="Overlay", Unit=UNIT_OVERLAY, TypeName="Selector Switch", Options=options, Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_TEXTCOLOR not in Devices:
            Domoticz.Device(Name="Text color", Unit=UNIT_TEXTCOLOR, TypeName="Color Switch", Switchtype=7, Image=Images["AWTRIXNG-Icons"].ID).Create()
        if UNIT_BRIGHTNESS not in Devices:
            Domoticz.Device(Name="Brightness", Unit=UNIT_BRIGHTNESS, TypeName="Dimmer", Image=Images["AWTRIXNG-Icons"].ID).Create()
